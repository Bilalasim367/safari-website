          ...
          <div className="bg-white rounded-b-lg border border-t-0 border-border p-6 min-h-[400px]">
            ...
            {activeTab === 'pricing' && (
              <div className="space-y-6">
                ...
                {/* Pricing is handled by Base Pricing card above - use price (sale) and originalPrice (MRP) */}
              </div>
            )}
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-4 sm:gap-6">
                      ...
                      <Label className="flex items-center gap-2 text-sm font-normal cursor-pointer">
                        <Checkbox checked={Boolean(form.watch('isTrending'))} onCheckedChange={(c) => form.setValue('isTrending', c as boolean)} />
                        Trending
                      </Label>
                      <Label className="flex items-center gap-2 text-sm font-normal cursor-pointer">
                        <Checkbox checked={Boolean(form.watch('isHotSelling'))} onCheckedChange={(c) => form.setValue('isHotSelling', c as boolean)} />
                        Hot Selling
                      </Label>
                    </div>
                  </CardContent>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // Auto-sync categorySlug based on gender selection
                    const categoryMap: Record<string, string> = {
                      Men: 'men',
                      Women: 'women',
                      Unisex: 'unisex'
                    };
                    if (categoryMap[genderValue]) {
                      form.setValue('categorySlug', categoryMap[genderValue]);
                    }
                  }}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                >
