...
export default function MobileFilterDrawer({
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="max-h-[80vh] p-0 rounded-t-2xl flex flex-col">
          ...
          <div className="flex items-center gap-3">
            {filterCount > 0 && (
              <Link
                ...
                className="text-sm text-[#9a958d] hover:text-[#B6965D] transition-colors underline underline-offset-2"
              >
                Clear All
              </Link>
            )}
        ...
        <div className="shrink-0 p-4 sm:p-6 border-t border-border bg-background">
          <SheetClose className="w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-[#B6965D] text-black hover:bg-[#B6965D]/90 h-11 px-4 py-2">
            Apply Filters
          </SheetClose>
        </div>
      </SheetContent>
